...
def save_latency_graph(rows, output_csv):
    """Plot every tested agent population explicitly and annotate measurements."""
    ...
    ax.set_title("Full SAMS latency as participant population grows")
...
def main():
    ap=argparse.ArgumentParser(description="Agent-count latency benchmark for the faithful dummy SAMS architecture.")
    ap.add_argument("--agents", default="1,4,8,16,24,48,96,192,394,788,985,1182,1379,1576,2364,3152")
    ap.add_argument("--warmup", type=int, default=6); ap.add_argument("--repeats", type=int, default=25)
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--out", default="agent_scaling_results.csv")
    ...
    for K in [int(x) for x in args.agents.split(",")]:
        ...
        rows.append({"num_agents":K, **r, "device":str(device)})
        print(rows[-1])
