...
def save_latency_graph(rows, output_csv):
    """Plot every tested latent width explicitly and annotate measured values."""
    labels = [str(row["latent_dim"]) for row in rows]
    x_pos = list(range(len(rows)))
    median = [row["median_ms"] for row in rows]
    p95 = [row["p95_ms"] for row in rows]
    params_m = [row["parameters"] / 1_000_000 for row in rows]
    ...
    ax.set_xlabel("Latent market-state dimension (exact tested width)")
    ax.set_ylabel("Forward latency (ms)")
    ax.set_title("Latency sensitivity to latent-state width")

    # Exact values on every point. Parameter count is included with median.
    for x, y, pm in zip(x_pos, median, params_m):
        ax.annotate(
            f"{y:.2f} ms\n{pm:.1f}M params",
    ...
    for x, y in zip(x_pos, p95):
        ax.annotate(
            f"P95 {y:.2f}",
            (x, y), xytext=(0, -17), textcoords="offset points",
    ...
    info = (
        ...
        f"action_dim={cfg.action_dim} | seq={cfg.sequence_len} | "
...
def main():
    ap=argparse.ArgumentParser(description="Latent-width latency benchmark.")
    ap.add_argument("--latent-dims", default="32,64,128,256,512,1024,2048,4096,8192,16384")
    ap.add_argument("--warmup", type=int, default=6); ap.add_argument("--repeats", type=int, default=25)
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--out", default="latent_scaling_results.csv")
    args=ap.parse_args(); device=torch.device(args.device)
    rows=[]
    for D in [int(x) for x in args.latent_dims.split(",")]:
        cfg=BenchConfig(latent_dim=D, num_agents=24)
        r=benchmark_model(cfg, device, args.warmup, args.repeats)
        rows.append({"latent_dim":D, **r, "device":str(device)})
        print(rows[-1])
