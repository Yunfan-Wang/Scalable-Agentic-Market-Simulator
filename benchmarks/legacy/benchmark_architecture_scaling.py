...
def benchmark_proposed(cfg, device, warmup=4, repeats=12):
    return benchmark_model(cfg, device, warmup, repeats)

def benchmark_naive_repeated_interpretation(cfg, device, warmup=2, repeats=8):
    """
    Naive baseline: each of K agents independently re-runs the large shared
    interpretation stack over the same market sequence. To avoid allocating
    K copies of identical weights, we reuse one encoder K times serially.
    This measures the repeated-compute penalty that the proposed design removes.
    """
    torch.manual_seed(7)
    one_cfg = BenchConfig(
        market_feature_dim=cfg.market_feature_dim,
        external_feature_dim=cfg.external_feature_dim,
        d_model=cfg.d_model,
        ...
        encoder_causal=cfg.encoder_causal,
        num_agents=1,
        agent_hidden=cfg.agent_hidden,
        action_dim=cfg.action_dim,
        target_dim=cfg.target_dim,
        sequence_len=cfg.sequence_len,
    )
    encoder = ConfigurableMarketEncoder(one_cfg).eval().to(device)
    population = VectorizedAgentPopulation(one_cfg).eval().to(device)
    transition = MarketTransitionHead(one_cfg).eval().to(device)
    market = torch.randn(1, cfg.sequence_len, cfg.market_feature_dim, device=device)
    external = torch.randn(1, cfg.sequence_len, cfg.external_feature_dim, device=device) if cfg.external_feature_dim > 0 else None

    def run_once():
        aggregate = torch.zeros(1, cfg.action_dim, device=device)
        for _ in range(cfg.num_agents):
            latents = encoder(market, external)
            hidden, previous = population.initial_state(1, device)
            pop = None
            for t in range(cfg.sequence_len):
                pop = population(latents[:, t], hidden, previous, deterministic=True)
                hidden, previous = pop["hidden"], pop["previous"]
            aggregate = aggregate + pop["aggregate_action"]
        aggregate = aggregate / cfg.num_agents
...
def save_comparison_graph(rows, output_csv):
    """Plot every K explicitly; annotate exact proposed and naive medians."""
    labels = [str(row["num_agents"]) for row in rows]
    x_pos = list(range(len(rows)))
    proposed = [row["proposed_median_ms"] for row in rows]
    naive = [row["naive_median_ms"] for row in rows]

    fig, ax = plt.subplots(figsize=(13.0, 7.0))
    ax.plot(x_pos, proposed, marker="o", linewidth=2.4,
            label="Proposed: 1 shared interpreter + K recurrent agents")
    ax.plot(x_pos, naive, marker="o", linewidth=2.4,
            label="Naive: K repeated interpretation stacks")

    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels, rotation=35, ha="right")
    ax.set_xlabel("Number of participant agents K (exact tested count)")
    ax.set_ylabel("Median forward latency (ms)")
    ax.set_title("SAMS computational scaling: shared vs repeated interpretation")

    for x, y in zip(x_pos, proposed):
        ax.annotate(f"{y:.2f} ms", (x, y), xytext=(0, 9),
                    textcoords="offset points", ha="center", fontsize=8)
    for x, y in zip(x_pos, naive):
        ax.annotate(f"{y:.2f} ms", (x, y), xytext=(0, -17),
                    textcoords="offset points", ha="center", fontsize=8)

    cfg = BenchConfig()
    info = (
        f"Fixed: input={cfg.market_feature_dim} | d_model={cfg.d_model} | "
        f"latent={cfg.latent_dim} | layers={cfg.layers} | heads={cfg.nhead} | "
        f"agent_hidden={cfg.agent_hidden} | action_dim={cfg.action_dim} | "
...
def main():
    ap=argparse.ArgumentParser(description="Shared-interpreter SAMS vs repeated-interpreter naive baseline.")
    ap.add_argument("--agents", default="1,4,8,16,24,48,96")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--out", default="architecture_scaling_comparison.csv")
    args=ap.parse_args(); device=torch.device(args.device)
    rows=[]
    for K in [int(x) for x in args.agents.split(",")]:
        cfg=BenchConfig(num_agents=K)
        p=benchmark_proposed(cfg, device, warmup=3, repeats=8)
        n=benchmark_naive_repeated_interpretation(cfg, device, warmup=1, repeats=4)
        row={
            "num_agents":K,
            "proposed_median_ms":p["median_ms"],
            "naive_median_ms":n["median_ms"],
            "speedup_vs_naive":n["median_ms"]/p["median_ms"],
            "device":str(device),
        }
        rows.append(row); print(row)
    save_results(rows, args.out, str(Path(args.out).with_suffix(".json")))
    save_comparison_graph(rows, args.out)
